...
def quentropy(A,scale, incomplete=True, base=2):
    ...
    if incomplete:
        A=A+[1-sum(A)]
    return scale*entropy(A,base=base)

def dist_entropy(dist,scale, incomplete=True, base=2):
    # scaled entropy. This function automatically completes the input set to 1; should this lead to errors due to rounding, use incomplete=False. Is designed to scale with the scaling factor of the binomial coefficient.
    A=[]
    for key in dist:
        A+=[dist[key]]
    return quentropy(A,scale, incomplete=incomplete, base=base)